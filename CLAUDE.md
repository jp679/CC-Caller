...
- `cc_caller/claude_worker.py` — sandboxed `claude -p` subprocess layer: deterministic session UUIDs, auto-recovery on session errors, judge prompts. `WORKER_SYSTEM_PROMPT` + `--disallowedTools` stop the worker Claude from invoking cc-caller/VAPI itself (a real bug that happened). Named sessions are created WITH their deterministic id (fresh_session_id) so name→id bindings survive across runs.
